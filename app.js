...
function loading(id) {
  ...
  if (el) el.innerHTML = '<div class="empty-state"><div class="empty-state-icon">⏳</div><div>Chargement...</div></div>';
}

// ===== ACCUEIL =====
async function renderAccueil() {
  var u = currentUser;
  loading('stats-row'); loading('recent-activity'); loading('todo-list');
  try {
    var cours = await DB.get('cours');
    var exercices = await DB.get('exercices');
    var devoirs = await DB.get('devoirs');
    var rendus = await DB.get('rendus');
    if (u.role === 'prof') {
      var aCorreger = rendus.filter(function(r) { return r.note === null; }).length;
      document.getElementById('stats-row').innerHTML =
        stat('👥', DB.getEleves().length, 'Élèves inscrits', '--brown') +
        stat('📚', cours.length, 'Cours publiés', '--blue') +
        stat('📋', aCorreger, 'À corriger', '--orange') +
        stat('🎯', exercices.length, 'Exercices', '--green');
      ...
      document.getElementById('stats-row').innerHTML =
        stat('📚', cours.length, 'Cours disponibles', '--blue') +
        stat('✏️', exercices.filter(function(e) { return e.type === 'exercice'; }).length, 'Exercices', '--green') +
        stat('📤', aRendre, 'Devoirs à rendre', '--orange') +
        stat('📋', exercices.filter(function(e) { return e.type === 'examen'; }).length, 'Examens', '--brown');
  ...
  } catch(e) { showToast('Erreur connexion Supabase', 'error'); console.error(e); }
}

function stat(icon, val, lab, color) {
  return '<div class="stat-card"><div class="stat-icon">' + icon + '</div><div class="stat-val" style="color:var(' + color + ')">' + val + '</div><div class="stat-lab">' + lab + '</div></div>';
}
function emptyState(icon, txt) {
  return '<div class="empty-state"><div class="empty-state-icon">' + icon + '</div><div>' + txt + '</div></div>';
}
...
async function renderCours() {
  ...
  } catch(e) { showToast('Erreur chargement cours', 'error'); console.error(e); }
}

async function openCours(id) {
  try {
    var c = await DB.getById('cours', id);
    if (!c) return;
...
async function renderExercices() {
  ...
  try {
    var all = await DB.get('exercices');
...
async function startExercice(id) {
  ...
  } catch(e) { showToast('Erreur chargement exercice', 'error'); }
}
...
function selectOpt(chosen, correct, exId) {
  ...
  document.querySelectorAll('.quiz-opt').forEach(function(el,i) {
...
async function nextQuestion(exId) {
  quizIndex++; quizAnswered = false;
  var ex = await DB.getById('exercices', exId);
...
async function renderTravaux() {
  ...
  try {
    ...
    if (u.role === 'eleve') {
      ...
      devoirs.forEach(function(d) {
        var nb = rendus.filter(function(r) { return r.devoirId === d.id; }).length;
  ...
  } catch(e) { showToast('Erreur chargement travaux', 'error'); console.error(e); }
}

async function simulateUpload(devoirId) {
  try {
    await DB.insert('rendus', { devoirId: devoirId, eleveId: currentUser.id, date: new Date().toISOString().slice(0,10), note: null, commentaire: null, fileName: 'travail_' + currentUser.id + '.pdf' });
    showToast('✅ Travail déposé !', 'success'); renderTravaux();
  } catch(e) { showToast('Erreur dépôt', 'error'); }
}

async function deleteDevoir(id) {
  if (!confirm('Supprimer ?')) return;
  await DB.delete('devoirs', id); renderTravaux(); showToast('Devoir supprimé');
}
...
async function renderExamens() {
  ...
  } catch(e) { showToast('Erreur chargement examens', 'error'); }
}
...
async function startExamConfirmed() {
  closeAllModals();
  var ex = await DB.getById('exercices', pendingExamId);
  if (!ex) return;
  var qs = Array.isArray(ex.questions) ? ex.questions : JSON.parse(ex.questions);
...
function selectExamOpt(i) {
  document.querySelectorAll('#exam-opts .quiz-opt').forEach(function(el,j) { el.classList.toggle('selected', j===i); });
...
function startExamTimer() {
  ...
  examTimerInterval = setInterval(function() {
    examState.remaining--;
    updateTimerDisplay();
    if (examState.remaining <= 0) { clearInterval(examTimerInterval); finishExam(true); }
  }, 1000);
}
...
function updateTimerDisplay() {
  ...
  var m = Math.floor(examState.remaining/60), s = examState.remaining%60;
  el.textContent = String(m).padStart(2,'0') + ':' + String(s).padStart(2,'0');
  el.classList.toggle('warning', examState.remaining < 120);
}

function stopExam() {
  clearInterval(examTimerInterval); examTimerInterval = null; examState = null;
  document.getElementById('examens-home').classList.remove('hidden');
  document.getElementById('exam-player').classList.add('hidden');
  renderExamens();
}
...
function finishExam(timeout) {
  ...
  var score = 0;
  examState.questions.forEach(function(q,i) { if (examState.answers[i]===q.correct) score++; });
  DB.addResultat(currentUser.id, examState.id, score, examState.questions.length, 'examen');
  var note = ((score/examState.questions.length)*20).toFixed(1);
  document.getElementById('exam-player').innerHTML = '<div style="max-width:500px"><div class="quiz-results"><div class="result-emoji">' + (timeout?'⏰':score>=examState.questions.length*0.6?'🎓':'📚') + '</div><div class="result-score">' + score + '/' + examState.questions.length + '</div><div class="result-sub">Note : <strong>' + note + '/20</strong></div><div style="margin-top:24px"><button class="btn-primary" onclick="stopExam()">Retour</button></div></div></div>';
  examState = null;
...
async function renderResultats() {
  ...
  try {
    var rendus = await DB.get('rendus');
    var rendusNotes = rendus.filter(function(r) { return r.eleveId === currentUser.id && r.note !== null; });
    if (!resultats.length && !rendusNotes.length) { document.getElementById('resultats-content').innerHTML = emptyState('📊', 'Aucun résultat pour l\'instant'); return; }
...
async function addCours() {
  ...
  try {
    ...
    await DB.insert('cours', item);
    pendingCoursFile = null; closeAllModals(); renderGestionCours(); showToast('✅ Cours ajouté !', 'success');
  } catch(e) { showToast('Erreur ajout cours', 'error'); console.error(e); }
}

// ===== FICHIERS =====
function readFileAsBase64(file) {
  return new Promise(function(resolve, reject) { var r = new FileReader(); r.onload = function(e) { resolve(e.target.result); }; r.onerror = reject; r.readAsDataURL(file); });
}
...
function setupDropZone(zoneId, inputId, labelId, onFile) {
  ...
  zone.addEventListener('drop', function(e) { e.preventDefault(); zone.classList.remove('drag-over'); if (e.dataTransfer.files[0]) handleDropFile(e.dataTransfer.files[0], labelId, onFile); });
  zone.addEventListener('click', function() { input.click(); });
  input.addEventListener('change', function() { if (input.files[0]) handleDropFile(input.files[0], labelId, onFile); });
}

function handleDropFile(file, labelId, onFile) {
  if (file.size > 4*1024*1024) { showToast('Fichier trop lourd (max 4 Mo)', 'error'); return; }
  var label = document.getElementById(labelId);
  if (label) label.textContent = '📎 ' + file.name + ' (' + formatSize(file.size) + ')';
  readFileAsBase64(file).then(function(data) { onFile({ fileData: data, fileName: file.name, fileMime: file.type }); });
}
...
function previewFileRaw(fileData, fileName, fileMime) {
  ...
  if (fileMime && fileMime.startsWith('image/')) content.innerHTML = '<img src="' + fileData + '" style="max-width:100%;border-radius:var(--radius)">';
  else if (fileMime === 'application/pdf') content.innerHTML = '<iframe src="' + fileData + '" style="width:100%;height:500px;border:none;border-radius:var(--radius)"></iframe>';
  else content.innerHTML = '<div style="text-align:center;padding:32px"><div style="font-size:48px">📄</div><div style="margin:12px 0">' + fileName + '</div><a href="' + fileData + '" download="' + fileName + '" class="btn-primary" style="text-decoration:none">⬇ Télécharger</a></div>';
...
async function previewFileSB(id, table) {
  try {
    var item = await DB.getById(table, id);
...
async function renderGestionExercices() {
  ...
  try {
    var exercices = await DB.get('exercices');
    document.getElementById('prof-exercices-list').innerHTML = exercices.map(function(e) {
      var qs = Array.isArray(e.questions) ? e.questions : (e.questions ? JSON.parse(e.questions) : []);
    ...
    }).join('') || emptyState('🎯', 'Aucun exercice');
  } catch(e) { showToast('Erreur', 'error'); }
}

async function deleteExercice(id) {
  if (!confirm('Supprimer ?')) return;
  await DB.delete('exercices', id); renderGestionExercices(); showToast('Supprimé');
}

function toggleExamMode() {
  document.getElementById('ne-duree-group').style.display = document.getElementById('ne-type').value === 'examen' ? 'block' : 'none';
}

function addQuestion() {
  var idx = questionBlocks.length;
  questionBlocks.push({ q: '', opts: ['','','',''], correct: 0, explication: '' });
  var letters = ['A','B','C','D'];
  var div = document.createElement('div');
  div.className = 'question-block'; div.id = 'qblock-' + idx;
  div.innerHTML = '<div class="question-block-header"><span class="question-num">Question ' + (idx+1) + '</span><button class="btn-secondary btn-sm" onclick="removeQuestion(' + idx + ')">Supprimer</button></div><div class="form-group"><label>Énoncé</label><textarea class="form-input" rows="2" placeholder="Question…" oninput="questionBlocks[' + idx + '].q=this.value"></textarea></div><div class="form-group"><label>Options (cochez la bonne réponse)</label>' + letters.map(function(l,i) { return '<div class="option-row"><div class="option-correct ' + (i===0?'selected':'') + '" id="oc-' + idx + '-' + i + '" onclick="setCorrect(' + idx + ',' + i + ')"></div><input class="form-input" placeholder="Option ' + l + '" oninput="questionBlocks[' + idx + '].opts[' + i + ']=this.value" style="flex:1"></div>'; }).join('') + '</div><div class="form-group"><label>Explication</label><input class="form-input" placeholder="Explication…" oninput="questionBlocks[' + idx + '].explication=this.value"></div>';
  document.getElementById('questions-list').appendChild(div);
}

function setCorrect(qIdx, optIdx) {
  questionBlocks[qIdx].correct = optIdx;
  [0,1,2,3].forEach(function(i) { var el = document.getElementById('oc-' + qIdx + '-' + i); if(el) el.classList.toggle('selected', i===optIdx); });
}
...
async function saveExercice() {
  ...
  try {
    ...
    await DB.insert('exercices', item);
    closeAllModals(); questionBlocks = []; pendingExerciceFile = null;
    document.getElementById('questions-list').innerHTML = '';
    renderGestionExercices(); showToast('✅ Exercice créé !', 'success');
  } catch(e) { showToast('Erreur création exercice', 'error'); console.error(e); }
}
...
async function openNoter(rendId) {
  noteTargetId = rendId;
  try {
    var r = await DB.getById('rendus', rendId);
    var devoirs = await DB.get('devoirs');
    var d = devoirs.find(function(x){return x.id===r.devoirId;});
    var e = DB.getUser(r.eleveId);
...
async function saveNote() {
  ...
  try {
    await DB.update('rendus', noteTargetId, { note: note, commentaire: document.getElementById('noter-comment').value });
    closeAllModals(); renderGestionTravaux(); showToast('✅ Note enregistrée !', 'success');
  } catch(e) { showToast('Erreur enregistrement note', 'error'); }
}
...
document.addEventListener('DOMContentLoaded', function() {
  var p = document.getElementById('ae-prenom'), n = document.getElementById('ae-nom');
  if (p) p.addEventListener('input', updateAeId);
  if (n) n.addEventListener('input', updateAeId);
});

function updateAeId() {
  var p = (document.getElementById('ae-prenom').value||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\s+/g,'');
  var n = (document.getElementById('ae-nom').value||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\s+/g,'');
...
function addEleve() {
  ...
  DB.insert('users', { id: id, nom: prenom + ' ' + nom, prenom: prenom, initiales: (prenom[0]+nom[0]).toUpperCase(), role: 'eleve', pw: document.getElementById('ae-pw').value || 'eleve123', classe: classe });
  closeAllModals(); renderGestionEleves(); showToast('✅ Compte créé : ' + id, 'success');
}

function deleteEleve(id) {
  if (!confirm('Supprimer ' + id + ' ?')) return;
  DB.delete('users', id); renderGestionEleves(); showToast('Élève supprimé');
}
...
async function addDevoir() {
  ...
  if (!titre) { showToast('Veuillez saisir un titre', 'error'); return; }
  try {
    await DB.insert('devoirs', { titre: titre, consignes: document.getElementById('nd-consignes').value, deadline: document.getElementById('nd-deadline').value || new Date(Date.now()+7*86400000).toISOString().slice(0,10), creePar: currentUser.id });
    closeAllModals(); renderTravaux(); showToast('✅ Devoir créé !', 'success');
...
function showToast(msg, type) {
  ...
  toastTimeout = setTimeout(function() { t.classList.add('hidden'); }, 3000);
}
